...
namespace Fractal_Terrain_Project
{
    ...
    public struct PointList
    {
		...
		public PointList(List<Point> ps)
		{
			points = ps;
		}
    	...
    	public Polyline ToPolyline()
    	{
    		...
    		{
    			polyline.verticies.Add(point);
    		}
    		
    		return polyline;
    	}
    	
		public void FromPolyline(Polyline polyline)
		{
    ...
    public struct Wireframe
    {
    	...
    	public Wireframe(List<Polyline> u, List<Polyline> v)
    	{
    		uCurves = u;
    		vCurves = v;
    	}
    ...
    public struct TriangleMesh
    {
    	...
    	public void MakeFromWireframe(Wireframe wireframe)
    	{
    		...
    		for (int i = 1; i < polylines.Count; i++)
    			...
    			{
    				triangles.Add(new Triangle(polylines[i-1][e-1], polylines[i][e-1], polylines[i][e]));
    				triangles.Add(new Triangle(polylines[i-1][e-1], polylines[i][e], polylines[i-1][e]));
    			}
    	}
    ...
    public struct QuadMesh
    {
    	...
    	public void MakeFromWireframe(Wireframe wireframe)
    	{
    		...
    		for (int i = 1; i < polylines.Count; i++)
    			for (int e = 1; e < polylines[i].Count; e++)
    				quads.Add(new Quad(polylines[i-1][e-1], polylines[i][e-1], polylines[i][e], polylines[i-1][e]));
    	}
...
//			    forwardAxis = Vector3D.CrossProduct(upAxis, rightAxis);
		...
		public void Yaw(double angle, ref Quaternion totalRotation, ref Quaternion localRotation)
		{
			angle *= Math.PI / 180;
			
			localRotation = Quaternion.FromAxisAngle(angle / 2, upAxis);
			//Output.Here("yaw", angle, localRotation);
			totalRotation *= localRotation;
		...
		public void Yaw(double angle, ref Quaternion totalRotation)
		{
			angle *= Math.PI / 180;
		...
		public void Pitch(double angle, ref Quaternion totalRotation, ref Quaternion localRotation)
		{
		    angle *= Math.PI / 180;
		...
		public void Pitch(double angle, ref Quaternion totalRotation)
		{
		    angle *= Math.PI / 180;
		...
		public void Roll(double angle, ref Quaternion totalRotation, ref Quaternion localRotation)
		{
		    angle *= Math.PI / 180;
			
		    localRotation = Quaternion.FromAxisAngle(angle / 2, forwardAxis);
			//Output.Here("roll", angle, localRotation);
			totalRotation *= localRotation;
		...
		public void Roll(double angle, ref Quaternion totalRotation)
		{
		    angle *= Math.PI / 180;
	...
	public static class Calculate
	{
		...
		public static Point CartesianPoint(double x, double y, double z = 0)
		{
			return new Point(x, y, z);
		}
		
		public static Point PolarPoint(double theta, double phi, double r)
		{
			return new Point(r * Math.Cos(theta) * Math.Sin(phi), r * Math.Sin(theta) * Math.Sin(phi), r * Math.Cos(phi));
		}
		
		public static Point CylindricalPoint(double theta, double z, double r)
		{
			return new Point(r * Math.Cos(theta), r * Math.Sin(theta), z);
		}
		
		public static Point FunctionResult(EquationType type, VariablesUsed func, double x, double y, double z)
		{
			switch (type)
			{
				case EquationType.CARTESIAN:
					switch (func)
					{
						...
						case VariablesUsed.ONE_THREE:
							return CartesianPoint(x, z, y);
						case VariablesUsed.TWO_THREE:
							return CartesianPoint(z, x, y);
					}
				...
				case EquationType.POLAR:
					...
					{
						case VariablesUsed.ONE_TWO:
							return PolarPoint(x, y, z);
						case VariablesUsed.ONE_THREE:
							return PolarPoint(x, z, y);
						case VariablesUsed.TWO_THREE:
							return PolarPoint(z, x, y);
					}
				...
				case EquationType.CYLINDRICAL:
					...
					{
						case VariablesUsed.ONE_TWO:
							return CylindricalPoint(x, y, z);
						case VariablesUsed.ONE_THREE:
							return CylindricalPoint(x, z, y);
						case VariablesUsed.TWO_THREE:
							return CylindricalPoint(z, x, y);
					}
